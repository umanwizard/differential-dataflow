...
pub struct MallocSizeOfOps {
    ...
    /// don't need it.
    pub have_seen_ptr_op: Option<Box<VoidPtrToBoolFnMut>>,
}
...
impl<T> MallocUnconditionalShallowSizeOf for std::rc::Rc<T> {
    fn unconditional_shallow_size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        let ptr: *const T = self.as_ref();
        unsafe { ops.malloc_size_of(ptr) }
    }
...
impl<T: MallocSizeOf> MallocUnconditionalSizeOf for std::rc::Rc<T> {
    fn unconditional_size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        self.unconditional_shallow_size_of(ops) + (**self).size_of(ops)
    }
}

impl<T> MallocConditionalShallowSizeOf for std::rc::Rc<T> {
    fn conditional_shallow_size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        let ptr: *const T = self.as_ref();
        if ops.have_seen_ptr(ptr) {
            0
...
impl<T: MallocSizeOf> MallocConditionalSizeOf for std::rc::Rc<T> {
    fn conditional_size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        let ptr: *const T = self.as_ref();
        if ops.have_seen_ptr(ptr) {
            0
